...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex min-w-[66px] items-center justify-center rounded-full px-3 py-1 text-xs font-semibold ${
        isActive
...
function SysOpManagementPage() {
  const allRows = useMemo(() => {
    return Array.from({ length: 152 }, (_, index) => {
      ...
      return {
        ...source,
        id: `SOP-${String(number).padStart(3, "0")}`,
      };
    });
  }, []);
      ...
      <div className="relative border-b border-cyan-300 bg-slate-50 px-5 py-5 dark:border-cyan-500/30 dark:bg-slate-900">
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end">
            <div className="grid flex-1 grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-[1.1fr_1.2fr_1fr]">
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-slate-50 text-left text-slate-600 dark:bg-slate-900 dark:text-slate-300">
              <th className="px-4 py-4 font-semibold">SysOp ID</th>
